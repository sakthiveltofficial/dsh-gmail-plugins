...
/**
 * Start one sign-in flow: launch the loopback server, build the consent URL,
 * open the browser, and return the flow. The server keeps running in the
 * background; call {@link awaitCallback} to complete the exchange.
 * @returns the created flow.
 */
export function startFlow({ clientId, clientSecret, scopes, timeoutMs }) {
